...
//                    plotData.plotLines[i] = PlotLine() // needed to insure refresh (see LegendView) makes reference change
...
//            pointStyle = symbolShapes.firstIndex(of: ShapeParameters(shape: plotLine.pointShape.shape, angle: plotLine.pointShape.angle, filled: false, color: .black)) ?? 0
...
//    .init(Polygon(sides: 4).path, filled: false), // Diamond
